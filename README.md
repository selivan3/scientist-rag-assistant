# Агент-помощник учёного на основе RAG+LLM

Учебный репозиторий для спецсеминара. Автор: Иван Селиванов.

Проект представляет разработанного мной помощника учёного: он работает с выбранными источниками, отвечает на вопросы только на основе подключённых материалов и помогает превращать корпус документов в понятные научные ответы.

## Что такое RAG+LLM в этом проекте

RAG+LLM здесь означает связку из двух частей:

- база знаний из источников, фрагментов и метаданных;
- языковая модель, которая получает найденные фрагменты и формирует ответ на русском языке.

Главная идея: модель не должна отвечать "из головы". Она должна опираться на найденные источники, показывать, откуда взят ответ, и честно писать, если данных не хватает.

## Скриншот

Ниже показан правильный интерфейс помощника: слева выбран источник, в центре ответ на вопрос, справа область заметок.

![Интерфейс RAG-помощника учёного](docs/screenshots/rag-assistant-interface.jpg)

## Демонстрация по рисункам курсовой

В демонстрацию добавлены все 10 рисунков из курсовой работы. Они лежат в [docs/figures](docs/figures), а полная русскоязычная галерея с пояснениями находится в [docs/demo.md](docs/demo.md).

### 1. Общая схема RAG

![Общая схема RAG](docs/figures/рисунок-1-rag.png)

### 2. Конвейер обработки источников

![Конвейер обработки источников](docs/figures/рисунок-1-конвейер.png)

### 3. Интерфейс модуля обработки документов

![Интерфейс модуля обработки документов через Paddle OCR](docs/figures/рисунок-2-интерфейс.png)

Этот рисунок относится к отдельному учебному модулю обработки документов через Paddle OCR. Он показывает не интерфейс RAG-помощника, а самостоятельную программу, которая распознаёт и подготавливает документы для дальнейшей работы с источниками.

### 4. Подготовка данных

![Подготовка данных](docs/figures/рисунок-2-подготовка.png)

### 5. База знаний

![База знаний](docs/figures/рисунок-3-знания.png)

### 6. Результат обработки документа

![Результат обработки документа через Paddle OCR](docs/figures/рисунок-3-результат.png)

Здесь показан результат работы отдельного модуля обработки документов через Paddle OCR, а не ответ RAG-помощника. Такой результат может использоваться как подготовленный источник для последующего поиска и анализа.

### 7. Демонстрация RAG-сценария

![Демонстрация RAG-сценария](docs/figures/рисунок-4-rag-демо.png)

### 8. Чёрно-белая схема конвейера

![Чёрно-белая схема конвейера](docs/figures/рисунок-4-конвейер-чб.png)

### 9. Структуризация материалов

![Структуризация материалов](docs/figures/рисунок-5-структуризация.png)

### 10. Агентная организация

![Агентная организация](docs/figures/рисунок-6-агенты.png)

## Связь с курсовой работой

Курсовая работа приложена в репозитории как подтверждающий учебный материал:

[Курсовая работа Ивана Селиванова](docs/coursework/coursework-selivanov.pdf)

Тема проекта связана с применением RAG-подхода для научной работы: загрузка источников, поиск по корпусу, ответы с опорой на документы и подготовка заметок.

## Возможности агента

- добавляет источник в базу знаний;
- разбивает текст на смысловые фрагменты;
- ищет релевантные фрагменты по вопросу;
- формирует ответ на русском языке;
- показывает, какие источники использованы;
- помогает вести заметки по прочитанному материалу.

## Архитектура

```mermaid
flowchart LR
    User["Пользователь"] --> Question["Вопрос"]
    Source["PDF, DOCX, заметки"] --> Ingest["Загрузка источников"]
    Ingest --> Chunks["Фрагменты текста"]
    Chunks --> Index["Индекс поиска"]
    Question --> Search["Поиск релевантных фрагментов"]
    Index --> Search
    Search --> Prompt["Контекст + вопрос"]
    Prompt --> Model["Языковая модель"]
    Model --> Answer["Ответ с опорой на источники"]
    Answer --> Notes["Заметки исследователя"]
```

## Минимальный демонстрационный запуск

В репозитории есть небольшой демонстрационный модуль без внешних зависимостей. Он показывает принцип поиска по локальным фрагментам.

```bash
PYTHONPATH=src python3 -m scientist_rag_assistant.demo "Что делает помощник учёного?"
```

Чтобы вывести список всех рисунков, используемых в демонстрации:

```bash
PYTHONPATH=src python3 -m scientist_rag_assistant.demo --figures
```

Направление развития: заменить демонстрационный поиск векторной базой и подключить языковую модель для полноценного RAG-сценария.

## Промпт помощника

Основная инструкция лежит в [prompts/scientist-prompt.md](prompts/scientist-prompt.md).

Ключевое правило:

> Отвечай только на основе найденных фрагментов. Если в источниках нет ответа, прямо скажи, что данных недостаточно.

## Ограничения

- В публичный репозиторий не добавляются чужие полные PDF-корпуса без проверки прав.
- Реальные ключи API не хранятся в репозитории.
- Если источник не содержит ответа, агент не должен заполнять пробел догадкой.

## Результат

Проект оформлен как отчёт по RAG+LLM-помощнику учёного: есть описание подхода, архитектура, правильный скриншот интерфейса, курсовая работа, все рисунки из демонстрации, промпт и минимальная демонстрация поиска по источникам.
