...
Этот рисунок относится к отдельному модулю обработки документов через Paddle OCR. Он показывает не интерфейс RAG-помощника, а программу, которая подготавливает документы для дальнейшей работы с источниками.
...
Здесь показан результат работы модуля обработки документов через Paddle OCR, а не ответ RAG-помощника. Такой результат может использоваться как подготовленный источник для последующего поиска и анализа.
